...
$$(ABI)_i=MR \times g(T_{med}) + \epsilon_i$$
where: 
...
is assumed to follows a Gamma distribution
...
## Guassian model 
set.seed(1234)
par1 <- list(MA = 400, OTmed = 10.5, b = 1.5, scale = 10)
par_hi1 <- list(MA = 500, OTmed = 20, b = 5, scale = 70)
par_lo1 <- list(MA = 0, OTmed = 5, b = 0.01, scale = 1)

start <- Sys.time()

result_mtabi1 <- anneal(
  model = mtabi1, par = par1,
  var = var, source_data = dtmed_abi,
  par_lo = par_lo1, par_hi = par_hi1,
  pdf = gamma_pdf,
  dep_var = "abi",
  initial_temp = 5, temp_red = 0.975, max_iter = 50000, 
...
likelihood::write_results(results = result_mtabi1, "output/models/mtabi1.txt")
```

```{r}
# Gaussian Extended 
set.seed(88)
par2 <- list(MA = 400, OTmed = 10.5, b = 2, a= 2, scale = 10)
par_hi2 <- list(MA = 700, OTmed = 20, b = 5, a = .5, scale = 100)
par_lo2 <- list(MA = 0, OTmed = 5, b = 1, a = 2.5, scale = 1)

start <- Sys.time()

result_mtabi2 <- anneal(
  model = mtabi2, par = par2,
  var = var, source_data = dtmed_abi,
  par_lo = par_lo2, par_hi = par_hi2,
  pdf = gamma_pdf,
  dep_var = "abi",
...
likelihood::write_results(results = result_mtabi2, "output/models/mtabi2.txt")
```
...
#Log-normal
set.seed(1234)
par3 <- list(MA = 400, OTmed = 10.5, b = 1.5, scale = 10)
par_hi3 <- list(MA = 700, OTmed = 20, b = 5, scale = 100)
par_lo3 <- list(MA = 0, OTmed = 5, b = 0.01, scale = 1)

start <- Sys.time()

result_mtabi3 <- anneal(
  model = mtabi3, par = par3,
  var = var, source_data = dtmed_abi,
  par_lo = par_lo3, par_hi = par_hi3,
  pdf = gamma_pdf,
  dep_var = "abi",
...
likelihood::write_results(results = result_mtabi3, "output/models/mtabi3.txt")
```

```{r}
set.seed(1234)
par4 <- list(MA = 400, a = 0, b = 0.5, c = -1, d = 10, scale = 10)
par_hi4 <- list(MA = 700, a = .5, b = 1, c = 2, d = 20, scale = 100)
par_lo4 <- list(MA = 0, a = -.5, b = 1.5, c = -2, d = 0, scale = 1)

start <- Sys.time()

result_mtabi4 <- anneal(
  model = mtabi4, par = par4,
  var = var, source_data = dtmed_abi,
  par_lo = par_lo4, par_hi = par_hi4,
  pdf = gamma_pdf,
  dep_var = "abi",
...
likelihood::write_results(results = result_mtabi4, "output/models/mtabi4.txt")
```

```{r}
## Asymptotic model 
set.seed(1234)
par5 <- list(MA = 400, Asym = 0.1, R0 = 1, lrc = -1, scale = 10)
par_hi5 <- list(MA = 700, Asym = 0.2, R0 = 10, lrc = 0, scale = 100)
par_lo5 <- list(MA = 0, Asym = 0, R0 = 0.5, lrc = -2, scale = 1)

start <- Sys.time()

result_mtabi5 <- anneal(
  model = mtabi5, par = par5,
  var = var, source_data = dtmed_abi,
  par_lo = par_lo5, par_hi = par_hi5,
  pdf = gamma_pdf,
  dep_var = "abi",
...
likelihood::write_results(results = result_mtabi5, "output/models/mtabi5.txt")
```
...
op1_abi <- get(paste0("result_", mod)) |> 
  op_plot(yvar = "abi", title = mod) + 
  xlim(0,600) + 
...
rp1_abi <- get(paste0("result_", mod)) |> 
  res_plot(yvar = "abi", title = mod)

mod <- "mtabi2"
aux_text <- all_results |> filter(modelo == mod)

op2_abi <- get(paste0("result_", mod)) |> 
  op_plot(yvar = "abi", title = mod) + 
  xlim(0,600) + 
  annotate(geom = "text", x= 0, y = 600, 
           label = bquote(R^2 == .(round(aux_text$R2,3))), hjust = 0) +
...
rp2_abi <- get(paste0("result_", mod)) |> 
  res_plot(yvar = "abi", title = mod)  

mod <- "mtabi3"
aux_text <- all_results |> filter(modelo == mod)

op3_abi <- get(paste0("result_", mod)) |> 
  op_plot(yvar = "abi", title = mod) + 
  xlim(0,600) + 
  annotate(geom = "text", x= 0, y = 600, 
           label = bquote(R^2 == .(round(aux_text$R2,3))), hjust = 0) +
...
rp3_abi <- get(paste0("result_", mod)) |> 
  res_plot(yvar = "abi", title = mod)  

mod <- "mtabi4"
aux_text <- all_results |> filter(modelo == mod)

op4_abi <- get(paste0("result_", mod)) |> 
  op_plot(yvar = "abi", title = mod) + 
  xlim(0,600) + 
  annotate(geom = "text", x= 0, y = 600, 
           label = bquote(R^2 == .(round(aux_text$R2,3))), hjust = 0) +
...
rp4_abi <- get(paste0("result_", mod)) |> 
  res_plot(yvar = "abi", title = mod)  
...
op5_abi <- get(paste0("result_", mod)) |> 
  op_plot(yvar = "abi", title = mod) + 
  xlim(0,600) + 
  annotate(geom = "text", x= 0, y = 600, 
           label = bquote(R^2 == .(round(aux_text$R2,3))), hjust = 0) +
...
rp5_abi <- get(paste0("result_", mod)) |> 
  res_plot(yvar = "abi", title = mod)  
 
```

### Resultados
#### Gaussian model (**mtabi1**) 
```{r}
#| fig.height: 8 
#| fig.width: 5
op1_abi / rp1_abi
...
```{r}
#| fig.height: 16
(op1_abi + rp1_abi) / (op2_abi + rp2_abi) / (op3_abi + rp3_abi) / (op4_abi + rp4_abi) / (op5_abi + rp5_abi)
```
...
## Comparing tmed - abi curves 
```{r}
#| fig.height: 6 
#| fig.width: 6
#| 
fit1 <- result_mtabi1$source_data |> mutate(modelo = "mtabi1")
fit2 <- result_mtabi2$source_data |> mutate(modelo = "mtabi2")
fit3 <- result_mtabi3$source_data |> mutate(modelo = "mtabi3")
fit4 <- result_mtabi4$source_data |> mutate(modelo = "mtabi4")
fit5 <- result_mtabi5$source_data |> mutate(modelo = "mtabi5")


fit <- bind_rows(fit1, fit2, fit3, fit4, fit5) 
  

colours_combined <- c("mtabi1" = "red", "mtabi2" = "blue", "mtabi3" = "green", "mtabi4" = "black", "mtabi5" = "darkgoldenrod4")


ggplot(fit, aes(x= tmed, y = abi, group = modelo, colour = modelo)) + 
  geom_point(colour="gray") + 
