...
df_predicted_prec <- df_predicted |> pivot_longer(-prec, names_to = "modelo", values_to = "pred") 

ggplot(
  (df_predicted_prec |> 
     filter(modelo != "mprecrat2")),
  aes(x = prec, y = pred, colour = modelo)) +
  geom_point(
    data = all_data, 
...
preceffect <- data.frame(prec = seq(0,1700, by =1)) |> 
  mutate(
    ...
    mprecrat5 = p4$Pa * exp(-p5$Pb * exp (-p5$Pc * prec))
  )
...
```{r}
var=list(mean = "predicted", x = "rat", prec = "wb", tmed = "tmed")  
```
...
```{r}
models_results |> 
  kbl(digits = c(0,0,2,0,2,2,3,3,2,3)) |> 
  kable_styling()
```
...
```{r}
x <- result_mcombrat1$likeli_hist

x |> slice_max(likeli) |> unique()

x |> filter(MR > 0.6) |> 
  ggplot(aes(x=MR, y=-likeli)) + geom_point()
...
support_limits(mcombrat1, result_mcombrat1$best_pars, 
               var = var, source_data = d, pdf = dnorm)


sqrt(diag(solve(-1*result_mcombrat1$hessian)))

result_mcombrat1$std_errs$*1.96
