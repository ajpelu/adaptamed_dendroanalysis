...
models_results <- all_results |> inner_join(all_time)
```
...
```{r}
#| fig.height: 16
(op1 + rp1) / (op2 + rp2) / (op3 + rp3) / (op4 + rp4)
```
...
fit <- bind_rows(fit1, fit2, fit3, fit4) |> rename(ratio_abi_npp = predicted, 
         mean_climate = tmed)

colours_combined <- c("mt1" = "red", "mt2" = "blue", "mt3" = "green", "mt4" = "black")
