...
avg_yearly <- bind_rows(
  ...
  summarise(
    annual_value = mean(value, na.rm=TRUE),
    annual_sd = sd(value, na.rm=TRUE),
    annual_se = annual_sd/sqrt(length(value))
  ),
  # prec
  d |>
  filter(var == "prec") |> 
...
write_csv(avg_yearly, "data/dendroadaptamed_climate_avg_yearly.csv")
```
