...
climate_diagram <- ggplot(
  ...
  geom_line() +
  ggh4x::facet_grid2(sp_code~factor(elev_code, levels=c('low', 'low2', 'medium', 'high')), 
                     render_empty = FALSE) +
...
climate_diagram_nolow2 <- ggplot(
    data = (avg_daily |> filter(elev_code != "low2") |> filter(var == "tmed")), 
      aes(x = date, y = mean*scale_factor)
    ) + 
  scale_x_date(date_breaks = "1 month", date_labels = "%b") + 
  geom_bar(
  ...
  theme_climate_diagram() + 
  xlab("") + 
  geom_text(data = (geodf |> filter(elev_code != "low2")), 
...
# performance::check_model(lm_tmed)
# performance::check_distribution(lm_tmed)

lm_tmed2 <- lm(avg ~ sp_code*elev_code, 
               data = (tmed_yearly2 |> filter(elev_code != "low2"))) 
...
means_letters_tmed <- bind_rows(
  ...
  generate_comparisons(lm_tmed, sp_code = "pinaster", elev_code = c("low", "low2", "medium", "high")),
  ...
  generate_comparisons(lm_tmed, sp_code = "sylvestris", elev_code = c("low", "medium", "high")),
   (emmeans(lm_tmed, specs = pairwise~elev_code) |> 
      multcomp::cld(Letters=letters, level=1-alpha) |> mutate(sp_code = "") |> 
      relocate(sp_code, .after = elev_code)),
  (emmeans(lm_tmed2, specs = pairwise~sp_code) |> 
     multcomp::cld(Letters=letters, level=1-alpha) |> 
     mutate(elev_code = "") |> 
     relocate(elev_code, .before = sp_code))
) |>  as.data.frame() |> 
  rename(letter = `.group`)
...
# eme 
model <- lm_tmed 
# generate_compi <- emmeans(model, 
#                           specs = paipairwise ~ sp_code * elev_code, 
#                           at = list(sp_code = "halepensis", elev_code = elev_code))

emm <- emmeans(model, pairwise ~ sp_code * elev_code, adjust = "bonferroni", alpha = alpha,
                 at = list(sp_code = "halepensis", elev_code = c("low", "medium", "high")))

letras <- multcomp::cld(emm, Letters=letters, level=1-alpha)
...
We found differences among species (Table \@ref(tab:tmed-post-sp)) and along the elevation (Table \@ref(tab:tmed-post-elev)). Temperature conditions are not different between the low2 and low, and also between low2 and mediumn elevations considering all the data. 



```{r}
# Function to generate comparisons and perform letter-based grouping with p-values
generate_comparisons <- function(model, sp_code = NULL, elev_code = NULL, alpha = 0.001) {
  emm <- emmeans(model, pairwise ~ sp_code * elev_code, adjust = "bonferroni", alpha = alpha,
                 at = list(sp_code = sp_code, elev_code = elev_code))
  cld_df <- multcomp::cld(emm, Letters = letters, level = 1 - alpha)
...
```{r tmed-post-elev}
TukeyHSD(aov_tmed, which = "elev_code")$elev_code |> 
  kbl(digits = c(0,3,3,3,4), 
      caption = "Posthoc differences among elevation for Tmed") |> 
  kable_paper("hover", full_width = F)
```
...
ph_tmed_sp <- glht(aov_tmed, linfct = mcp(sp_code = "Tukey"))
old.par <- par(mai=c(1.5,2,1,1))
plot(confint(ph_tmed_sp))
...
emmeans(lm_tmed, specs=~elev_code*sp_code)

a <- emmip(lm_tmed, elev_code~sp_code)
...
```{r}
ph_tmed_elev <- glht(aov_tmed, linfct = mcp(elev_code = "Tukey"))
old.par <- par(mai=c(1.5,2,1,1))
plot(confint(ph_tmed_elev))
...
tabla_postHoc <- function(modelo){
  posthoc <- emmeans(modelo, ~ sp_code * elev_code)
...
tabla_postHoc(aov_tmed)

emmeans(tmed_aov, avg ~ sp_code)
...
```{r}
posthoc <- model_emmeans(lm_tmed, at = c("sp_code", "elev_code")) |> 
  pairs()
parameters::parameters(posthoc)
```


report::report(tmed_aov)

parameters::parameters(tmed_aov)


modelbased::model_emmeans(tmed_aov, at = c("sp_code")) |> pairs()
parameters::parameters(posthoc)



estimate_contrasts(tmed_aov, contrast = c("sp_code"), length=2)
contrasts


# Not normal 
plot(aov_tmed_yearly, which = 2)
car::qqPlot(aov_tmed_yearly$residuals)

library(modelbased)


model <- aov(avg ~ sp_code*elev_code, data = tmed_yearly) 
m <- estimate_contrasts(model)
...
```{r}
stat.test <- tmin_yearly |> 
  group_by(sp_code) |> 
  t_test(avg ~ elev_code) %>%
  adjust_pvalue(method = "bonferroni") %>%
  add_significance("p.adj")
stat.test

bxp <- ggboxplot(
  tmin_yearly, x = "sp_code", y = "avg", 
  color = "elev_code"
  )

# Add p-values onto the box plots
stat.test <- stat.test %>%
  add_xy_position(x = "sp_code", dodge = 0.8)
bxp + stat_pvalue_manual(
  stat.test,  label = "p.adj", tip.length = 0, remove.bracket = TRUE
  )
