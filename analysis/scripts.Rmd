...
- **Temporal series of Vegetation Indexes** NDVI and EVI from MOD13Q1. We used the following GEE code to get the EVI and NDVI data for each field plot. We obtained a total of 503 values for each field plot (2000-2021, 23 images per year, except 2000 year (only 20 images)). The data were saved as `dendroadaptamed_iv_mod13q1.csv` (see [data/dendroadaptamed_iv_mod13q1.csv](https://github.com/ajpelu/adaptamed_dendroanalysis/blob/master/data/dendroadaptamed_iv_mod13q1.csv)). The temporal resolution was one image each 16 days (23 images per year), and the spatial resoultion was 250 m.  
...
var modNpp = ee.ImageCollection('MODIS/006/MOD17A3HGF')
  .filterDate(startdate, enddate)
  .select(['Npp', 'Npp_QC']); 
