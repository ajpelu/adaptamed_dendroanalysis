...
mk_prec <- aux |> 
  ...
  mutate(ypos = 
           case_when(
             elev_code == "low" ~ 1300,
             elev_code == "low-Dec" ~ 1200,
             elev_code == "medium" ~ 1400,
             elev_code == "high" ~ 1500), 
         p_value_string = symnum(pvalue_mk, corr = FALSE,
               cutpoints = c(0,  .001,.01,.05, 1),
               symbols = c("***","**","*","")),
...
figura_ts_clima <- clima |> 
  ...
  geom_text(x = 1950, aes(y = ypos, label = taulabel), data = mk, parse = TRUE, show.legend = FALSE, hjust = "left", size = 4.5)
