...
fig_clima <- ggplot(aux, aes(y = tmed, x = prec, colour = sp_code)) +
  ...
  guides(
    ...
    shape = guide_legend(override.aes = list(color = "black", fill = "white"))) +
  annotation_custom(
  grid::rasterGrob(img_pines), xmin = 700, xmax = 2000, ymin = 13, ymax = 16
) + 
  annotate("text", x=900, y=12.7, label= "italic(halepensis)", parse = TRUE) +
  annotate("text", x=1250, y=12.7, label= "italic(pinaster)", parse = TRUE) +
  annotate("text", x=1550, y=12.7, label= "italic(nigra)", parse = TRUE) +
  annotate("text", x=1870, y=12.7, label= "italic(sylvestris)", parse = TRUE)
