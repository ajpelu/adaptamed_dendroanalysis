...
```{r}
stand <- read_csv("data/dendroadaptamed_stand_summary.csv") |>
...
ggsave(
  p_evi_lai, 
  file = "output/figure_evi_lai.jpg",
  dpi = 400,
...
nls_model_lai_ba <- nls(lai ~ a + b * log(bac), data = summary_evi, 
                 start = list(a = 0.2, b = 0.1))

observed <- summary_evi$lai
# Sum of Squared Residuals (SSR)
SSR <- sum((observed - predict(nls_model_lai_ba))^2)
# Total Sum of Squares (TSS)
TSS <- sum((observed - mean(observed))^2)
# R-squared
r2_lai_ba <- 1 - (SSR / TSS)
...
p_lai_ba <- p_lai_ba + geom_smooth(data = summary_evi,
                          method = "nls", 
              formula = y ~ a + b * log(x), 
              method.args = list(start = list(a = 0.2, b = 0.1)), 
              se = FALSE, colour = "black", linetype = "dashed") +
  annotate("text", x = 15, y = 3.5, label= sprintf("R^2 == %.3g", round(r2_lai_ba, 2)),
           color="black",
           parse = TRUE, 
           size = 6)
p_lai_ba
