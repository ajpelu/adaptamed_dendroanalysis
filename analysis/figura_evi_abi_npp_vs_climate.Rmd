---
title: "Figure NPP EVI ABI vs water balance"
date: "2024-03-24"
...
plot_wb <- 
  ...
  scale_fill_manual(values = colours_sp, name = "Species") + 
  theme_bw() + 
  geom_smooth(aes(linetype = sig, linewidth = sig), method = "lm", se = FALSE) +
  scale_linetype_manual(values = lines_lm, guide = "none") +
  scale_linewidth_manual(values = lines_lm_size, guide = "none") +
  geom_smooth(aes(), 
              method = "nls", 
              formula=y~SSlogis(x, Asym, xmid, scal),
              se =  FALSE, # this is important 
