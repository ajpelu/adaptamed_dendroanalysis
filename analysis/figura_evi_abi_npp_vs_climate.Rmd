...
plot_wb <- 
  ...
  facet_wrap(~factor(y_variable2, c("Annual~EVI[Landsat]", 
                                    "Annual~NPP[MODIS]~(g~C~m^2~year^{-1})",
                                    "ABI~(g~C~m^2~year^{-1})")),
             scales = "free_y", 
...
letras_wb <- data.frame(
  y_variable2 = c("Annual~EVI[Landsat]", "Annual~NPP[MODIS]~(g~C~m^2~year^{-1})", "ABI~(g~C~m^2~year^{-1})"),
  label = c("a", "b", "c"),
...
plot_wb <- 
  ...
  filter(climate_variable == "water_balance") |> 
  ggplot(aes(x=mean_climate, y = mean_y, colour = sp_code)) + 
  geom_point(aes(shape = elev_code, fill = sp_code), alpha = alpha_points, size = size_points) +
  scale_colour_manual(values = colours_sp, name = "Species") +
  scale_shape_manual(values = shape_elev, name = "Elevation") +
  scale_fill_manual(values = colours_sp, name = "Species") + 
  theme_bw() + 
