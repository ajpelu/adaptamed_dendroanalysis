...
ggsave(
  main_figure2, 
  file = "output/figure_model_selected2.jpg",
  dpi = 400,
  width = 7.09*2.2, height = 7.09*2.2/2
)
```
...
```{r}
# Crear datos ficticios solo para generar las leyendas
dummy_data <- data.frame(
  prec = 1:4,  # Valores ficticios para prec (no se usarán)
  tmed = 1:4,  # Valores ficticios para tmed (no se usarán)
  elev_code = factor(c("low-Dec", "low", "medium", "high"), levels = c("low-Dec", "low", "medium", "high")),
  species = factor(c("P. halepensis", "P. pinaster", "P. nigra", "P. sylvestris"), levels = c("P. halepensis", "P. pinaster", "P. nigra", "P. sylvestris"))
)

# Crear un gráfico vacío solo con las leyendas para Species y Elevation
plot_vacio <- ggplot(dummy_data, aes(x = prec, y = tmed, shape = elev_code, colour = species)) + 
  geom_point(size = 0, alpha = 0) + 
  scale_shape_manual(
    values = shape_elev, 
    name = "Elevation", 
    guide = guide_legend(override.aes = list(size = 3, alpha = 1, fill = NA))) +  
  scale_colour_manual(
    values = colours_Specie, 
    labels = expression(italic("P. halepensis"), italic("P. pinaster"), italic("P. nigra"), italic("P. sylvestris")),
    name = "Species", 
  ...
  theme(
    legend.position = "bottom",
    legend.box = "vertical",   # Organizar las leyendas en una fila
...
surface_plot  <- plot_with_ABI_legend / plot_vacio + 
  plot_layout(heights = c(11, .4))  # Ajustar el espacio de las leyendas y el gráfico
...
ggsave(
  surface_plot, 
  file = "output/figure_surface.jpg",
  dpi = 300,
  width = 3.46*1.5, height = 3.46*1.7
)
```
...
#### Histogram 
```{r}
ggplot(d, aes(y = rat, fill = species)) +
  geom_point(aes(x = 0, shape = elev_code),
             position = position_jitter(width = 0.02, height = 0), size = 2) +  # Puntos apilados
  geom_density(alpha = 0.6) +  # Curva de densidad
  facet_wrap(~ species, ncol = 4) +  # Facetado por species
  theme_minimal() +
  scale_shape_manual(
    values = shape_elev, 
    name = "Elevation")
...
```{r}
shape_elev <- c(21, 22, 23, 24)  # Puedes ajustar estos valores según las formas que desees usar

# Crear el gráfico con puntos apilados y densidad por elevación, facetado por especie
ggplot(d, aes(y = rat, fill = species)) +
  geom_point(aes(x = 0, shape = elev_code), position = position_jitter(width = 0.02, height = 0), size = 2) +  # Puntos apilados
  geom_density(aes(color = elev_code), alpha = 0.6, adjust = 1) +  # Curva de densidad por elev_code
  facet_wrap(~ species, ncol = 4) +  # Facetado por species
  labs(title = "Density Plot and Stacked Points of RAT by Species and Elevation", x = "RAT", y = "Density") +
  theme_minimal() +
  scale_shape_manual(values = shape_elev, name = "Elevation")  # Definir las formas para elev_code
