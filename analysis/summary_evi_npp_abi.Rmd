...
ggsave(
  plot_avg_eviabinpp, 
  file = "output/plot_evi_abi_npp_gradient.png",
  dpi = 350,
  width = 5, height = 6.5
)   
...
out_sp <- bind_rows(letter_abi, letter_evi, letter_npp)
...
out_sp <- df_avg_sp |> 
  inner_join(
    bind_rows(letter_abi, letter_evi, letter_npp)
  ) |> 
  mutate(pvalue = ifelse(p_value < 0.0001, "<0.0001", round(p_value, 3))) 
```
...
```{r}
writexl::write_xlsx(out_sp, "output/tables/summary_evi_abi_npp_species.xlsx")
```
