...
letras_indicativo_plot <- data.frame(
  variable2 = c("Annual~EVI[Landsat]", 
...
out_plot2 <- out_plot |> inner_join(letras_indicativo_plot, by="variable2")


plot_avg_eviabinpp <- ggplot(data = out_plot2, 
       aes(y = mean, x = sp_code, group = elev_code, colour = elev_code)) + 
  geom_jitter(data = df_plot,
              aes(y = mean_y, x = sp_code, 
  ...
  geom_point(size = 2.2, position = custom_position, stroke = .1,
             aes(fill = elev_code), show.legend = FALSE) +
...
ggsave(
  plot_avg_eviabinpp, 
  file = "output/plot_evi_abi_npp_ratio_gradient.png",
  dpi = 350,
  width = 5, height = 8.5
)   
...
generate_comparisons_sp <- function(model, sp_code = NULL, alpha = 0.001) {
  emm <- emmeans(model, pairwise ~ sp_code , adjust = "bonferroni", alpha = alpha,
                 at = list(sp_code = sp_code))
...
x <- df |> filter(y_variable == "evi_landsat")  
lm_evi <- lm(mean_y ~ sp_code , data = x) 

x <- df |> filter(y_variable == "abi")  
lm_abi <- lm(mean_y ~ sp_code , data = x) 

x <- df |> filter(y_variable == "npp")  
lm_npp <- lm(mean_y ~ sp_code , data = x) 

x <- df |> filter(y_variable == "ratio")  
lm_ratio <- lm(mean_y ~ sp_code , data = x) 



letter_evi <- generate_comparisons_sp(lm_evi, sp_code = c("halepensis","pinaster", "nigra", "sylvestris")) |> mutate(y_variable = "evi_landsat")
letter_abi <- generate_comparisons_sp(lm_abi, sp_code = c("halepensis","pinaster", "nigra", "sylvestris")) |> mutate(y_variable = "abi")
letter_npp <- generate_comparisons_sp(lm_npp, sp_code = c("halepensis","pinaster", "nigra", "sylvestris")) |> mutate(y_variable = "npp")
letter_ratio <- generate_comparisons_sp(lm_ratio, sp_code = c("halepensis","pinaster", "nigra", "sylvestris")) |> mutate(y_variable = "ratio")

out_sp <- bind_rows(letter_abi, letter_evi, letter_npp, letter_ratio)



df_avg_sp <- df |> 
  group_by(y_variable, sp_code) |> 
  summarise(mean = mean(mean_y), 
            sd = sd(mean_y), 
            se = sd/sqrt(length(mean_y))) |> ungroup()
...
out_sp <- df_avg_sp |> 
  inner_join(
    bind_rows(letter_abi, letter_evi, letter_npp, letter_ratio)
  ) |> 
  mutate(pvalue = ifelse(p_value < 0.0001, "<0.0001", round(p_value, 3))) 
```
...
out_sp |> 
  ungroup() |> 
  dplyr::select(-p_value) |> 
  arrange(y_variable, sp_code) |> 
  kbl(digits = c(0,0,3,3,3, 3,3,0,3,3,0,5), 
      caption = "Posthoc differences among species for average ABi, NPP and EVI") |> 
    kable_paper("hover", full_width = F)
```

```{r}
writexl::write_xlsx(out_sp, "output/tables/summary_evi_abi_npp_ratio_species.xlsx")
```
